...
extension ReceiptTableViewController {
  ...
  private func cellForItemsSection(_ indexPath: IndexPath) -> UITableViewCell {
    ...
    let fakeAssModel = ReceiptItem(itemName: "BABA BOOEY", itemCost: Amount(rawValue: 1499), isBillableToPayer: Array(repeating: true, count: billModel.persons.count))
  ...
  private func cellForAdjustmentsSection(_ indexPath: IndexPath) -> UITableViewCell {
    ...
    let fakeAssAdjustment = ReceiptAdjustment(adjustmentName: "tip", adjustment: .percentage(15.0.percent, .runningTotal))
